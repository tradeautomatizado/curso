...
class AddressDatabase(object):
    ...
    def add_record(self, last_name="", other_names="", email_address=""):
        ...
        c = db.cursor()
        c.execute(
            "INSERT INTO records(last_name, other_names, email_address) \
                    VALUES(?,?,?)",
            (last_name, other_names, email_address),
        )
        db.commit()
        c.close()
    ...
    def update_record(self, record_id, last_name="", other_names="", email_address=""):
        ...
        c = db.cursor()
        c.execute(
            "UPDATE records set last_name=?, other_names=?, email_address=? \
...
class RecordList(npyscreen.MultiLineAction):
    def __init__(self, *args, **keywords):
        ...
        self.add_handlers({"^A": self.when_add_record, "^D": self.when_delete_record})
    ...
    def on_ok(self):
        if self.record_id:  # We are editing an existing record
            ...
            self.parentApp.myDatabase.add_record(
                last_name=self.wgLastName.value, other_names=self.wgOtherNames.value, email_address=self.wgEmail.value,
            )
